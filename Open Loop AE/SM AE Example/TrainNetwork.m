function [txNet,rxNet,info,trainedNet] = ...
  TrainNetwork(n,k,c,SNR,normalization,EbNo,KFactorLin,varargin)
...
R = bits_transmitted/n; 
  ...
  trainParams.MiniBatchSize = 128;
  trainParams.InitialLearnRate = 0.001;
...
frame_length = 1000;
numTrainSymbols = frame_length * 80; 
numValidationSymbols = frame_length * 10; 
numTestSymbols = frame_length * 10;
...
train_data = readmatrix('datasets\c1d2_rician10\train_data.csv'); %[data pathGains]
validation_data = readmatrix('datasets\c1d2_rician10\validation_data.csv'); %[data pathGains]
...
options = trainingOptions('adam', ...
  ...
  'L2Regularization',trainParams.L2Regularization);
