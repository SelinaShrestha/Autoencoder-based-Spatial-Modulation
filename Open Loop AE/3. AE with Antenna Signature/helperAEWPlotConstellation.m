function helperAEWPlotConstellation(trainedNet,encoder_layer,varargin)
...
M_d = trainedNet.Layers(2).numOutputs; % Data Layer
Nt = trainedNet.Layers(3).numOutputs; % Control Layer
...
%inputSymbols = eye(M);
inputSymbols = [repelem(eye(Nt),M_d,1) repmat(eye(M_d),Nt,1) zeros(Nt*M_d,2*Nt)];

if strcmp(method, 'interleaved')
  y = activations(trainedNet, inputSymbols, encoder_layer);
  if encoder_layer == "SMLayer"
        y= [sum(y(1:Nt,:)); sum(y(Nt+1:2*Nt,:))]; 
  end
  y2D = reshape(y,2,[]);
  plot(ax,y2D(1,:) + 1i* y2D(2,:),'*','Tag','aew_constellation','LineWidth',1.5,'MarkerSize',12)
  grid(ax,"on")
  axis(ax,"equal")
  axis(ax,"square")
  ax.FontSize = 20;
  axis([-1.5 1.5 -1.5 1.5])
  xlabel(ax,'In phase','FontSize',20)
  ylabel(ax,'Quadrature','FontSize',20)
  ...
  if useNoisySamples
    ...
    inputSymbols = [repelem(eye(Nt),M_d,1) repmat(eye(M_d),Nt,1)];
    y = activations(trainedNet, inputSymbols, encoder_layer);
    % Removing SM to just get the learned constellation
    if encoder_layer == 'SMLayer'
        y= [sum(y(1:Nt,:)); sum(y(Nt+1:2*Nt,:))]; 
    end
    y = awgn(y,10);
