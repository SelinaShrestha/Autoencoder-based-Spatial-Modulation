function [txNet,rxNet,info,trainedNet] = ...
  TrainNetwork_power_constraint_encoder2(n,k,c,SNR,Pmax,normalization,EbNo,KFactorLin,varargin)
...
DataSymbols = zeros(numTrainSymbols+numValidationSymbols,M);
DataSymbols(sub2ind([numTrainSymbols+numValidationSymbols, M],...
    (1:numTrainSymbols+numValidationSymbols)',d+1)) = 1;
...
lgraph_1 = connectLayers(lgraph_1, 'relu_1', 'fc_6');
lgraph_1 = connectLayers(lgraph_1, 'fc_6', 'relu_6');
lgraph_1 = connectLayers(lgraph_1, 'relu_6', 'fc_2');
...
for idxChan = idxNorm:length(trainedNet.Layers)
  ...
  %if isa(trainedNet.Layers(idxChan), 'ConcatenationLayer')
    break
  end
end
